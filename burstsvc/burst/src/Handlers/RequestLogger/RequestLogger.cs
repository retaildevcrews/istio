...
namespace Ngsa.Middleware
{
    ...
    public class RequestLogger
    {
            ...
            if (App.Config.Prometheus)
            {
                requestHistogram = Metrics.CreateHistogram(
                            ...
                            new HistogramConfiguration
                            {
                                ...
                                LabelNames = new string[] { "code", "region", "zone" },
                            });
        ...
        private static void LogRequest(HttpContext context, double ttfb, double duration)
        {
            ...
            if (App.Config.Prometheus && requestHistogram != null)
            {
                requestHistogram.WithLabels(GetPrometheusCode(context.Response.StatusCode), App.Config.Region, App.Config.Zone).Observe(duration);
                requestSummary.WithLabels(GetPrometheusCode(context.Response.StatusCode), App.Config.Region, App.Config.Zone).Observe(duration);
            }
        }
