...
impl RootContext for RootHandler {
    ...
    fn on_configure(&mut self, _config_size: usize) -> bool {
        ...
        // !! totally Not-So-Production code
        match env::var("NGSA_NAMESPACE") {
            Ok(val) => self.config.namespace = val,
            Err(e) =>  error!("{:?}", e),
        }

        match env::var("NGSA_HPA_NAME") {
            Ok(val) => self.config.deployment = val,
            Err(e) =>  error!("{:?}", e),
        }
